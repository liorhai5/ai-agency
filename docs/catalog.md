...
| **AI Tooling Architect** | `@ai-tooling-architect` | AI development infrastructure specialist — MCP server design, skill packaging and distribution, plugin lifecycle, instruction delivery pipelines, prompt cache plumbing, and multi-client portability. | You're designing an MCP server and need tool schema guidance. You're packaging a skill for multi-client distribution. You're optimizing prompt cache hit rates or context-window plumbing. You're building a plugin install/update/uninstall lifecycle. |
...
| **Code Reviewer** | `@code-reviewer` | Provides constructive, prioritized code reviews focused on correctness, security, maintainability, and performance. Marks each finding by severity (Critical/Major/Minor/Nit) with a calibrated confidence score, and suppresses findings it can't trace to a quoted line. Never style-polices. | You've written code and want a thorough second set of eyes before it ships. You want feedback framed as teaching, not gatekeeping. You need specific, actionable comments — not vague "security issue" notes. |
...
| **AI Engineer** | `@ai-engineer` | LLM call reliability advisor — prompt protocol design and validation, token economics, structured output reliability, RAG architecture, hallucination detection, and evaluation methodology. | You're designing a prompt protocol and want it validated against success criteria. You need a per-item token budget. You're measuring extraction reliability, schema compliance, or hallucination rates. You're choosing between RAG, fine-tuning, and prompt engineering for one LLM step. |
| **Agentic Flow Architect** | `@agentic-flow-architect` | Agentic workflow and AI-agent harness designer — pattern choice (single agent → router → parallel → evaluator → graph → multi-agent), memory scopes, trust boundaries, gates, observability, and eval drift. Defaults to the simplest reliable pattern. | You're choosing between a single-agent pipeline and a multi-agent supervisor. You need trust boundaries and gates named before shipping an autonomous loop. You want a coverage check across product, access, data, ops, governance, and evolution before a flow goes to production. |
