...
Parse the arguments passed from the dispatcher:
- Tokens starting with `@` → agent names (strip the `@` prefix). These are filename stems matching files in `agents/`.
- Everything else → task description.
- If no `@` tokens found → error: "Specify at least one agent using @name syntax. Run `/agency list` to see available agents."
- If no task text → infer the task from the current conversation context.

## Agent Resolution

For each `@name`:
1. Read the agent file at `agents/{name}.md`.
2. If the file is not found → list files in the `agents/` directory and search for substring matches of the given name. If matches are found, suggest them: "Agent '@name' not found. Did you mean: @match1, @match2?" If no substring matches either, error: "Agent '@name' not found. Run `/agency list` to see available agents."

## Execution — Single Agent

When exactly one agent is specified:

1. Read the agent's full file content from `agents/{name}.md`.
...
1. For each agent, spawn a **parallel subagent** using the Agent tool with this prompt:
